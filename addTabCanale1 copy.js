...
//stato diverso  Draft, Da Approfondire, Oltre Capacity e Annullata    
      if (!(stato == 2 || stato == 6 || stato == 8 || stato == 9)) {
    ...
    $('#campaign_state_id').on('select2:select', function() {
      alert(' stato campagna dentro addCanale  ' + $('#campaign_state_id').val());
      alert(' selected_channel_id1  ' + $('#channel_ins1').val());
      
      
            stato = $('#campaign_state_id').val();
            count = document.getElementById('iniziative_dealer').value;          
            //validazione_addCanale(selected_channel_id1, stato);              
            //validazione_addCanaleDealer(count);
