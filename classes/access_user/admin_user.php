...
class Admin_user extends Access_user {
	...
	function update_user_by_admin($new_level, $user_id, $def_pass, $new_email, $active, $confirmation = "no") {
        ...
        function update_user_by_admin2($new_level, $user_id, $def_pass, $new_email ="no", $active, $confirmation = "no") {
		$this->user_found = true;
		$this->user_access_level = $new_level;
                
                $update_sql = "UPDATE `users` SET `lastname`=".$_POST['cognome'].",`firstname`=".$_POST['nome'].",`login`=".$_POST['username'].",`job_role_id`=".$_POST['selectRuolo'].",`squad_id`=".$_POST['selectDipartimento'].",`active`=".$_POST['selectStato'].",`leggi`=Yes,`inserisci`=".$_POST['inserisci'].",`modifica`=".$_POST['modifica'].",`cancella`=".$_POST['cancella'].",`email`='xxx',`tmp_mail`=xxx,`pw`=".$_POST['password'].",`access_level`=1 WHERE `id`=".$_POST['idUtente']." ";
