...
//echo $sql;
 ...
 function tablePianificazioneData($list) {   
    ...
    $user_info['squad_id'] = $page_protect->get_squad();
     ...
     foreach ($list as $key => $row) {
        ...
        ?>
        <form action="index.php?page=inserisciCampagna2" method="post" id="campagnaModifica<?php echo $row['id'];?>">
 ...
 function tableGestione($list) {    
    ...
    function getFilter(){
        ...
        //$this->reset_filter_session_new();

        if(isset($_POST)){
            $filter_view = $_POST;        
             //echo 'dentro IF POST prima ';
        print_r($_POST);    
        }
        else{    
            $filter_view = $_SESSION['filter'];
            
         //echo 'dentro IF SESSION getFilter prima';
        //print_r($_SESSION); 
        }
        ...
        //echo '<br>RETURN SESSION getFilter DOPO';
        //print_r($_SESSION); 
