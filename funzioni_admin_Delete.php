...
//echo $query3;
    ...
    left join campaign_groups on campaign_groups.id=campaign_types.campaign_group_id
    where campaign_groups.id=$id";
//echo $query3;
        $result3 = $this->mysqli->query($query3) or die($query3 . " - " . $this->mysqli->error);
        $list = array();
        $r = array();
...
//echo $query3;
    ...
    function get_group_label($id) {
        $query3 = "SELECT campaign_groups.id,campaign_groups.NAME as tipo_nome,campaign_groups.label as tipo_label FROM campaign_groups 
...
//echo $query3;
    ...
    function get_all_list($nome_tabella, $order_condition = 'ORDER BY `campaign_types`.`campaign_group_id` ASC', $filter = "") {
    ...
    #SELECT * FROM `campaign_types` WHERE 1 ORDER BY `campaign_types`.`campaign_group_id` ASC
    ...
    function update_name($nome_tabella, $id, $new_value, $color = NULL, $elimina = NULL, $label = NULL, $description = NULL) {
        ...
        } elseif ($nome_tabella == "campaign_types" || $nome_tabella == 'campaign_groups' || $nome_tabella == 'channels' || $nome_tabella == 'segments') {
    ...
    function insert_new_campaigntype($name, $id_group, $label) {
        $query3 = "INSERT INTO `campaign_types`(`id`, `name`, `campaign_group_id`,label) VALUES ( NULL, '$name', '$id_group','$label')";
        $result3 = $this->mysqli->query($query3) or die($query3 . " - " . $this->mysqli->error);
...
/*
	...
	*/
    ...
    function users_get_list($nome_tabella) {
        $query3 = "SELECT users.id, `lastname`, `firstname`, `login`, `job_role_id`, `department_id`, `active`, `leggi`, `inserisci`, `modifica`, `cancella`  , job_roles.name  as ruolo, departments.vendor as dipartimento FROM `users` left join departments on `department_id`=departments.id left join job_roles on `job_role_id`=job_roles.id ORDER BY `users`.`lastname` ASC";
        $result3 = $this->mysqli->query($query3) or die($query3 . " - " . $this->mysqli->error);
    ...
    function users_get_list_where_old($where) {
        $query3 = "SELECT users.id, `lastname`, `firstname`, `login`, `job_role_id`, `department_id`, `active`, `leggi`, `inserisci`, `modifica`, `cancella` ,maillist , job_roles.name  as ruolo, departments.vendor as dipartimento FROM `users` left join departments on `department_id`=departments.id left join job_roles on `job_role_id`=job_roles.id $where ORDER BY `users`.`lastname` ASC";
    ...
    function users_get_list_where($where) {
        $query3 = "SELECT users.id, `lastname`, `firstname`, `login`, `job_role_id`, `email`, `department_id`, `active`, `leggi`, `inserisci`, `modifica`, `cancella` ,maillist , job_roles.name  as ruolo, departments.name as dipartimento FROM `users` left join departments on `department_id`=departments.id left join job_roles on `job_role_id`=job_roles.id $where ORDER BY `users`.`lastname` ASC";
        #$query3 = "SELECT * From `users` $where ORDER BY `users`.`lastname` ASC";
        $result3 = $this->mysqli->query($query3) or die($query3 . " - " . $this->mysqli->error);
    ...
    function user_update($query) {
        #$query3 = "UPDATE `users` SET `lastname`=".$_POST['cognome'].",`firstname`=".$_POST['nome'].",`login`=".$_POST['username'].",`job_role_id`=".$_POST['selectRuolo'].",`department_id`=".$_POST['selectDipartimento'].",`active`=".$_POST['selectStato'].",`leggi`=Yes,`inserisci`=".$_POST['inserisci'].",`modifica`=".$_POST['modifica'].",`cancella`=".$_POST['cancella'].",`email`='xxx',`tmp_mail`=xxx,`pw`=".md5($_POST['password']).",`access_level`=1 WHERE `id`=".$_POST['idUtente']." ";
